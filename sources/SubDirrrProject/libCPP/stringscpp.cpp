...
char** StringsCPP::initialize_text(int rows, int max){
    char** text;
...
char* StringsCPP::initialize_string(int max){
    return (char*) malloc(max*sizeof(char));
}

void StringsCPP::input_text(char** text, int rows, int max){
    int i;
...
void StringsCPP::print_text(char** text, int rows){
    int i;
    for (i = 0; i<rows; i++)
        printf("%s\n", text[i]);
}

int StringsCPP::get_length(char* string){
    int len = 0;
    while(*string++!=0) len++;
    return len;
}

int StringsCPP::count_spaces(char* string){
    int count = 0;
    while(*string++!=0) count+=(*string==' '?1:0);
    return count;
}

int StringsCPP::count_chars(char* string, char chr){
    int count = 0;
    while(*string++!=0) count+=(*string==chr?1:0);
    return count;
}

int StringsCPP::get_max_string_length(char** text, int rows){
...
char* StringsCPP::insert_char(char* str, int place, char chr){
    int i;
...
char* StringsCPP::insert_chars(char* str, int place, char chr, int count){
    int i;
    for(i = 0; i<count; i++)
...
void StringsCPP::get_string(char *str, int max){
    int i = 0, ch;
    while((ch = getchar()) != '\n')
        if(str != NULL && i < max - 1)
            str[i++] = ch;
...
int StringsCPP::get_char_index(char* str, char chr, int num){
    int i, temp = 0;
...
void StringsCPP::spread_text(char** text, int rows){
